...
export const addNewBook = async(newTitle, newStart, newEnd) => {
    //store response of POST request for new book called new Title to read between newStart and newEnd
    //fetch() function is used to connect to specified API endpoint /books
    ...
    //will wait for the response in json format
    const newBook = await response.json(); 
    return newBook;
};
// TODO: Create the getBooks function that retrieves all of the books that have been saved to the back-end API
export const getBooks = async() => {
    //we are using await to wait from response from API before returning the list of items
    const response = await fetch(`${API_ENDPOINT}/books`);
